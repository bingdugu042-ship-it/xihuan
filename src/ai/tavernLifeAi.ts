...
export async function generateCommissionBoard(settings: AppSettings): Promise<CommissionJob[]> {
  ...
  try {
    const raw = await askJson(
      settings,
      '你是艾泽利亚村庄「委托会」告示牌文书。只输出 JSON，不要 markdown。',
      `生成 3 条可接日工委托：
{
  "jobs": [
    {
...
reward 为金币整数 20-200；staminaCost 为 1 或 2。`,
    )
    const parsed = extractJson<{
      jobs?: {
        title?: string
...
export async function resolveCommissionRun(
  ...
  try {
    const raw = await askJson(
      settings,
      '你是艾泽利亚委托结算旁白。只输出 JSON。结果可成功、部分成功或失败（一无所获）。',
      `旅者「${playerName}」接取委托：
标题：${job.title}
雇主：${job.employer}
简介：${job.summary}
...
输出：
{
  "narrative": "过程叙述 120-220 字",
  "outcome": "success|partial|fail",
  "coinsDelta": 0,
  "staminaSpent": ${job.staminaCost}
}
fail 时 coinsDelta 必须为 0；partial 约为标价的 30%-60%；success 接近标价。`,
    )
    const parsed = extractJson<{
      narrative?: string
      outcome?: string
